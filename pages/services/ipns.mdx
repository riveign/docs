...
<Tabs items={['CLI', 'SDK', 'UI']}>
  <Tab>
    To create an IPNS record using the CLI you need to be authenticated, with a project seelcted and use the `create` command.
...
Publishing an IPNS record means advertising it so that other nodes can resolve1 it, thus mapping the record to a CID so that the gateway can resolve it.

To publish an IPNS record you can do as follows

<Tabs items={['CLI', 'SDK', 'UI']}>
  <Tab>
    To publish an IPNS record with a CID using the CLI you need to be authenticated, with a project selcted and use the `publish` command.
    ```shellscript filename="PCreating an IPNS record" copy
...
<Tabs items={['CLI', 'SDK', 'UI']}>
  <Tab>
    To resole an IPNS record using the CLI you need to be authenticated, with a project seelcted and use the `resolve` command.
    ```shellscript filename="PCreating an IPNS record" copy
